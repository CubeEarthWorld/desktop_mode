...
このアプリは、外部ディスプレイへのミラーリングにしか対応していない端末で、コンテンツが端末側と外部ディスプレイ側の両方に二重表示されることを避けるために開発されました。端末側をコンテンツ表示ではなく外部ディスプレイ専用のタッチパッドとして使い、外部ディスプレイ側を操作対象の画面として利用することを目的としています。

root、Shizuku、ADB常駐、非公開APIには依存しません。Androidの公開マルチディスプレイAPIとAccessibilityServiceを使用します。

> [!IMPORTANT]
> このプロジェクトは開発中です。Android 16（API 36）以上であっても、connected displays / desktop windowingの実装は端末メーカーによって異なります。すべての端末やアプリでの動作は保証されません。
...
署名済みのインストール用APKは[最新のGitHub Release](https://github.com/CubeEarthWorld/external-display-touchpad/releases/latest)からダウンロードできます。

1. ReleaseのAssetsから`external-display-touchpad-v1.0.0.apk`をダウンロードします。
2. Android端末で、ダウンロードに使用したアプリからの「不明なアプリのインストール」を許可します。
3. APKを開いてインストールします。
4. 初回起動後、案内に従ってAccessibilityServiceを有効にします。

同じAPKとSHA-256チェックサムは、リポジトリ内の[`releases/`](releases/)にも収録しています。

> [!NOTE]
> 開発用debug版をインストール済みの場合、署名が異なるため正式版へ直接更新できません。debug版をアンインストールしてから正式版をインストールしてください。アンインストールするとアプリ設定は削除されます。
...
release APKを独自にビルドする場合は、[`android/key.properties.example`](android/key.properties.example)を参考に署名鍵を設定してください。秘密鍵と実際の`android/key.properties`はGitへ追加しないでください。
