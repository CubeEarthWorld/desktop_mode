...
ComponentName ごとに次の `AppWindowMode` を保存する。

- `auto`: manifest の固定向きを判定する。
- `phonePortrait`: 本体ディスプレイの短辺:長辺を使用する。
- `phoneLandscape`: 本体ディスプレイの長辺:短辺を使用する。
- `fullExternal`: 起動範囲を指定しない。
...
- 設定 schema version は2。
- 新規既定の長押し時間は1000ms。
- schema version 1で値が旧既定550msの場合だけ1000msへ移行する。
...
- 電話比率、縦横モード、手動上書き、起動範囲非対応・無視の診断を確認する。
