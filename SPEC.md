...
| R8 | **2本指タップ判定 120ms は実用上厳しい。** また2本→1本遷移後の move がカーソル移動として誤処理される | 判定窓を **350ms** に緩和。タッチセッション単位の**ステートマシン**(§6)を定義し、2本指解除後の move は全て無視 |
...
class AppSettings {
  ...
  final int longPressDurationMs;   // 400–800、初期 450
...
| `stopSession` | — | — | |
| `moveCursor` | `{dx, dy}`(論理px、フレーム集約済み) | — | native が pointerSpeed を掛けて反映 |
| `leftClick` | — | — | 現在カーソル位置に 50ms tap |
| `longPress` | — | — | 現在カーソル位置に longPressDurationMs の長押し(右クリック相当) |
| `pointerDown` | — | — | ドラッグ開始(カーソル位置で短い接触) |
| `pointerMove` | `{dx, dy}` | — | ドラッグ中の移動 |
| `pointerUp` | — | — | ドラッグ終了 |
| `twoFingerScrollStart` | — | — | 2本指スクロール開始(カーソル中心に仮想2点を置く) |
| `twoFingerScrollBy` | `{dx, dy}` | — | 2本指スクロールの今フレーム分移動量 |
| `twoFingerScrollEnd` | — | — | 2本指スクロール終了 |
| `twoFingerSwipe` | `{dx, dy}` | — | 2本指スワイプ（フリック）方向をカーソル位置から dispatch |
| `systemAction` | `{action: "back"\|"home"}` | `bool` | 外部ディスプレイ上のナビゲーションジェスチャー |
| `updateConfig` | `{pointerSpeed, longPressDurationMs, showCursor, cursorIdleTimeoutMs, ...}` | — | **R1: 新規追加** |
...
- `tapSlop=12 logical px`
- `twoFingerWindowMs=350` (2本目を認識する猶予)
- `longPressDurationMs`(設定値、既定 450ms。静止長押し→ドラッグ/長押し判定の閾値)
- `swipeMinVelocity=1.0 logical px/ms` (2本指をスワイプとみなす最小重心速度)
- `swipeMinDistance=24 logical px` (2本指をスワイプとみなす最小重心移動距離)
...
single
 ...
 ├ up:   !moved && 経過 < 220ms → leftClick → idle
 │       !moved && longPressDurationMs 経過後 → longPress → idle
 │       それ以外 → idle
 ├ 静止したまま longPressDurationMs 経過 → longPressElapsed フラグを ON
 └ 2本目 down: 1本目から350ms以内 && !moved → twoFinger
              それ以外 → dead
dragging(長押し後の move で遷移)
 ├ move → DragMoveResult
 └ up → DragEndResult → idle
twoFinger
 ├ いずれかの指の移動 → 速度で判定
 │   · 重心速度 < swipeMinVelocity または 累積距離 < swipeMinDistance
 │     → twoFingerScroll(TwoFingerScrollStartResult + TwoFingerScrollMoveResult)
 │   · 重心速度 >= swipeMinVelocity かつ 累積距離 >= swipeMinDistance
 │     → twoFingerSwipe(以後スクロールは送出せず、up で TwoFingerSwipeResult)
 └ 全指 up(未移動) → 何も送らず idle(右クリックは廃止)
twoFingerScroll(ゆっくり移動 = スクロール)
 ├ move → 重心移動量を TwoFingerScrollMoveResult として送出
 │          移動が速くなれば twoFingerSwipe へ遷移し TwoFingerScrollEndResult を送出
 └ いずれかの指 up → TwoFingerScrollEndResult → idle/dead
twoFingerSwipe(素早いスライド = スワイプ)
 ├ move → 何も送出しない(方向ベクトルを追跡のみ)
 └ いずれかの指 up → TwoFingerSwipeResult(重心の総移動量) → idle/dead
dead(誤動作防止: カーソル移動もクリックも送らない)
 └ 全指 up → idle

cancel(システムによる ACTION_CANCEL): dragging/twoFingerScroll/twoFingerSwipe 中であれば
 up と同じ終了結果(DragEndResult/TwoFingerScrollEndResult)を必ず返す。
 これを怠ると native 側の直列化ガードが解放されず、以後 busy に固着する
 (v1.3 で修正した実機不具合の根本原因、§-1 参照)
```

- **クリック判定は指を離すタイミングで行う。** シングルタップ(短時間・未移動)は `LeftClickResult`、長押し後に未移動で離すと `LongPressResult`、長押し時間経過後の移動は `DragStartResult` となる。
- **デルタ集約(R2):** move 中のデルタを controller 内で加算し、フレームコールバックで 1 回だけ `moveCursor`/`pointerMove`/`twoFingerScrollBy` を呼ぶ
- 3本以上のタッチは即 `dead`
